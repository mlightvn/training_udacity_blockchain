/**
 *          BlockchainController
 * 
 * This class expose the endpoints that the client applications will use to interact with the 
 * Blockchain dataset
 */
class BlockchainController {

    //The constructor receive the instance of the express.js app and the Blockchain class
    constructor(app, blockchainObj) {
        this.app = app;
        this.blockchain = blockchainObj;

        this.init();
    }

    init() {
        // This method will initialize the endpoints
        this.app.get("/", (req, res) => {
            res.send("Welcome to the Blockchain API");
        });

        // All the endpoints methods needs to be called in the constructor to initialize the route.
        this.getBlockByHeight();
        this.requestOwnership();
        this.getSignatureMessage();
        this.submitStar();
        this.getBlockByHash();
        this.getChainSize();
        this.getStarsByOwner();
    }

    // Enpoint to Get a Block by Height (GET Endpoint)
    getBlockByHeight() {
        this.app.get("/block/height/:height", async (req, res) => {
            if(req.params.height) {
                const height = parseInt(req.params.height);
                try {
                    let block = await this.blockchain.getBlockByHeight(height);
                    if(block){
                        return res.status(200).json(block);
                    } else {
                        return res.status(404).send({result: "Block Not Found!"});
                    }
                } catch (error) {
                    return res.status(404).send({error});
                }
            } else {
                return res.status(404).send({result: "Block Not Found! Review the Parameters!"});
            }
            
        });
    }

    // Endpoint that allows user to request Ownership of a Wallet address (POST Endpoint)
    requestOwnership() {
        this.app.post("/request-ownership", async (req, res) => {
            if(req.body.address) {
                const address = req.body.address;
                const message = await this.blockchain.requestMessageOwnershipVerification(address);
                if(message){
                    return res.status(200).json(message);
                } else {
                    return res.status(500).send({result: "An error happened!"});
                }
            } else {
                return res.status(500).send({result: "Check the Body Parameter!"});
            }
        });
    }

    getSignatureMessage() {
        this.app.post("/message-signature", async (req, res) => {
            if(req.body.address && req.body.message) {
                const privateKey = req.body.private_key;
                const address = req.body.address;
                const message = req.body.message;
                try {
                    const signature = await this.blockchain.getSignatureMessage(privateKey, address, message);
                    if(signature){
                        return res.status(200).json({signature});
                    } else {
                        return res.status(500).send({result: "An error happened!"});
                    }
                } catch (error) {
                    return res.status(500).send({error});
                }
            } else {
                return res.status(500).send({result: "Check the Body Parameter!"});
            }
        });
    }

    // Endpoint that allow Submit a Star, you need first to `requestOwnership` to have the message (POST endpoint)
    submitStar() {
        this.app.post("/submit-star", async (req, res) => {
            if(req.body.address && req.body.message && req.body.signature && req.body.star) {
                const address = req.body.address;
                const message = req.body.message;
                const signature = req.body.signature;
                const star = req.body.star;
                try {
                    let block = await this.blockchain.submitStar(address, message, signature, star);
                    if(block){
                        return res.status(200).json(block);
                    } else {
                        return res.status(500).send({result: "An error happened!"});
                    }
                } catch (error) {
                    console.log({error: error});
                    return res.status(500).send({error});
                }
            } else {
                return res.status(500).send({result: "Check the Body Parameter!"});
            }
        });
    }

    // This endpoint allows you to retrieve the block by hash (GET endpoint)
    getBlockByHash() {
        this.app.get("/block/hash/:hash", async (req, res) => {
            if(req.params.hash) {
                const hash = req.params.hash;
                let block = await this.blockchain.getBlockByHash(hash);
                if(block){
                    return res.status(200).json(block);
                } else {
                    return res.status(404).send({result: "Block Not Found!"});
                }
            } else {
                return res.status(404).send({result: "Block Not Found! Review the Parameters!"});
            }
            
        });
    }

    // This endpoint allows you to request the list of Stars registered by an owner
    getChainSize() {
        this.app.get("/blocks/size", async (req, res) => {
            try {
                let size = await this.blockchain.getChainSize();
                return res.status(200).json({size: size});
            } catch (error) {
                return res.status(500).send({result: "An error happened!"});
            }
        
        });
    }

    // This endpoint allows you to request the list of Stars registered by an owner
    getStarsByOwner() {
        this.app.get("/blocks/:address", async (req, res) => {
            if(req.params.address) {
                const address = req.params.address;
                // return res.status(200).json({address});
                try {
                    let stars = await this.blockchain.getStarsByWalletAddress(address);
                    if(stars){
                        return res.status(200).json(stars);
                    } else {
                        return res.status(404).send({result: "Block Not Found!"});
                    }
                } catch (error) {
                    return res.status(500).send({result: "An error happened!"});
                }
            } else {
                return res.status(500).send({result: "Block Not Found! Review the Parameters!"});
            }
            
        });
    }

}

module.exports = (app, blockchainObj) => { return new BlockchainController(app, blockchainObj);}